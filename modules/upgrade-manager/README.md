...
Manual controls upgrade the selected candidate, the highest-ranked candidate, all currently filtered eligible candidates by one tier, or all filtered eligible candidates toward a user-selected target level with one confirmation. Quick Upgrade defaults to one level above the lowest healthy eligible item and shows the remaining production time for both aggregate and selected upgrades. Buildings display Tiberium/Power, offense displays Crystal/Power, and defense displays Tiberium/Crystal/Power; Credits are not presented as upgrade costs. Upgrades only occur in direct response to a user action. A capped activity log records those manual actions.

...
Manual controls upgrade the selected candidate, the highest-ranked candidate, or all currently filtered eligible candidates. Upgrades only occur in direct response to a user action; the module does not automatically upgrade bases, buildings, defense, or offense. A capped activity log records those manual actions.

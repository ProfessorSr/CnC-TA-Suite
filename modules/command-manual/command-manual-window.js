...
function sectionHtml(entry, module) {
  const metadata = module
    ...
    : '';
  const related = (RELATED[entry.id] ?? []).map((id) => MANUAL_SECTIONS.find((item) => item.id === id)?.title ?? id);
...
export function manualSearch(query) {
  const normalized = String(query ?? '').trim().toLowerCase();
  if (!normalized) return MANUAL_SECTIONS.map((entry) => ({ id: entry.id, label: entry.title }));
  const results = [];
  for (const entry of MANUAL_SECTIONS) {
...
export function buildCommandManual(context, owner = null) {
  ...
  const moduleButton = new qx.ui.form.Button('Installed Modules');
  ...
  const renderArticle = (id) => {
    const entry = MANUAL_SECTIONS.find((item) => item.id === id) ?? MANUAL_SECTIONS[0];
    currentId = entry.id;
    article.setValue(sectionHtml(entry, moduleFor(entry.id)));
    const index = visible.findIndex((item) => item.id === currentId);
    previous.setEnabled(index > 0);
    next.setEnabled(index >= 0 && index < visible.length - 1);
  };
  const searchMatches = () => manualSearch(currentQuery)
    .map((result) => ({ entry: MANUAL_SECTIONS.find((entry) => entry.id === result.id), label: result.label }))
  ...
  search.addListener('changeValue', (event) => {
    ...
    currentQuery = query;
    visible = query ? MANUAL_SECTIONS.filter((entry) => searchText(entry).includes(query)) : [...MANUAL_SECTIONS];
    renderContents();
  });
  ...
  moduleButton.addListener('execute', () => {
    article.setValue('<div style="padding:14px;background:#c8d3d7;color:#17262d;border-top:4px solid #edf5f7;border-bottom:6px solid #667a83;border-radius:8px">'
      + '<h1 style="color:#075d7a;margin-top:0">Installed Modules</h1>'
  ...
  root.__suiteManualOpen = (id) => {
    search.setValue?.('');
    currentQuery = '';
    visible = [...MANUAL_SECTIONS];
    renderContents();
    renderArticle(MANUAL_SECTIONS.some((entry) => entry.id === id) ? id : 'welcome');
  };

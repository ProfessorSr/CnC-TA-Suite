...
export const MANUAL_SECTIONS = Object.freeze([
  ...
  section('war-room', 'War Room',
    ...
    ['Enter any target’s attack screen; War Room makes that live target authoritative.', 'Use Search to find targets and optionally download CSV, copy an in-game coordinate list, or open a Communications draft.', 'Choose an Attack Simulator objective such as CY, DF, maximum RP, or destruction.', 'Move troops in the game attack screen.', 'Run simulations and use play to show the game’s native animation.', 'Open Report Summary to inspect each attack and hand a selected row to the native report/replay viewer.', 'Save useful formations and reload them later.'],
    [['Search', 'Finds bases, camps, outposts, or alliance targets by level and CP range.'], ['Formation Controls palette', 'Appears only in native attack setup. Drag its native-themed panel to reposition it; the Suite remembers the location. Its icons move, mirror, swap, hide/show, reset, save, simulate, or open War Room.'], ['Single-troop toggle', 'Enable the crossed-unit icon, click as many troops as needed to hide or unhide them, then click the icon again to leave the mode.'], ['Manual layout result', 'Moving troops in the native attack screen queues a simulation. Each completed distinct layout appears immediately as a cached Battle Simulator result with replay and formation controls.'], ['Simulate', 'Runs the current formation through the native simulator.'], ['Simulate & Play', 'Runs and displays native combat animation.'], ['Report Summary row', 'Reviews one native attack inside War Room; double-click to open the game report.'], ['Open Native Report / Replay', 'Opens the selected attack in the game viewer.'], ['Return to Attack Setup', 'Returns the game to editable attack formation.'], ['Save Formation', 'Persistently stores the current offense arrangement for this attacking base and this exact target; prepared targets receive a faction-colored map plate.'], ['Load Formation', 'Restores a saved arrangement only when its original attacking base and target are active.'], ['Apply/Move Troops', 'Moves the live formation only after the user initiates it.']],
  ...
  section('combat-reports', 'Combat Reports',
    'Filters and aggregates PvE/PvP report history, loot, experience, repairs, outcomes, and trends.',
    ['Open Combat Reports.', 'Choose report type, outcome, target, or date filters.', 'Sort the report table.', 'Inspect totals and averages.', 'Open a native report where available.'],
    [['Filters', 'Select report subsets.'], ['Refresh', 'Reads report data again.'], ['Open', 'Opens the native report when available.']]),
